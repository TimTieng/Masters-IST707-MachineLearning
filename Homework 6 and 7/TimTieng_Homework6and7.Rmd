---
title: "Tim Tieng Homework 6 & 7"
output:
  pdf_document: default
  html_document:
    df_print: paged
  # Attestation 
  # I used the code snippets and report examples provided in the IST 707 Repo to complete this assignment. 
---
...
get_accuracy_rate(table(all_results$orig, all_results$pred), length(all_results$pred))
```
Based off the output, we can see that the k-means value of 3 returned the highest accuracy rate of 91.93%
...
get_accuracy_rate(table(all_results$orig, all_results$pred), length(all_results$pred))
```
Polynomial Kernel = 13.27% Accuracy rate
```{r}
# Radial Kernel
all_results <- data.frame(orig=c(), pred=c())
for (k in 1:kfolds) {
